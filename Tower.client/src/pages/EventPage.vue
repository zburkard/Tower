<template>
        ...
        <div :style="`background-image: url(${event.coverImg})`" class="row mt-3 rounded-bottom elevation-4 cover">
            <div class="col-11">
                ...
                <div v-else class="row justify-content-end m-3 align-items-center">
                    <div class="col-2 d-flex justify-content-around">
                        <h5>Tickets Left:</h5>
                        <h5 v-if="event.capacity <= 20" class="text-danger">
                            {{ event.capacity }}
                        </h5>
                        <h5 v-else>{{ event.capacity }}</h5>
                    </div>
                    <button v-if="event.capacity == 0" disabled class="col-2 p-2 btn btn-success">
                        <h5 class="fw-bold">Event is Sold Out⚠</h5>
                    </button>
                    <button v-else-if="account.id && !foundMe" @click="buyTicket()" class="col-2 p-2 btn btn-success">
                        <h5 class="fw-bold">Get A Ticket 🎟</h5>
                    </button>
                    <button v-else-if="account.id" @click="removeTicket(foundMe.id)" class="col-2 p-2 btn btn-danger">
            ...
            <div class="col-1 mt-3">
                <button v-if="account.id == event.creatorId && event.isCanceled == false" @click="cancelEvent(event.id)"
                    ...
                    Event</button>
                <button v-else-if="event.isCanceled == true" class="btn btn-outline-danger fw-bold fs-4" disabled>Event
                    Canceled</button>
            </div>
        </div>
    </div>
    <div class="container">
        ...
        <div class="row justify-content-center">
            <div class="col-10 dark rounded-pill">
                <div class="m-2" v-for="t in ticketHolders">
                    <img :src="t.profile.picture" alt="" class="picture rounded-circle elevation-4">
                </div>
            </div>
        </div>
    </div>
                ...
                <form @submit.prevent="createComment()">
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1" class="text-light fs-4">Leave A Comment<img
                                v-for="t in ticketHolders" :src="t.profile.picture"
                                class="picture rounded-circle elevation-4 m-3" alt=""></label>
